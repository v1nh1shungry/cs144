...
size_t ByteStream::write(const string &data) {
    auto len = min(data.size(), _capacity - _buffer.size());
    copy(data.cbegin(), data.cbegin() + len, back_inserter(_buffer));
    _write_count += len;
    return len;
}
...
bool ByteStream::eof() const { return _ended && buffer_empty(); }
